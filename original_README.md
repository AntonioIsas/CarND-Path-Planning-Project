...
The frenet system provides some easy coordinates to follow the lanes more easily, while the spline tool lets us generate good trajectories with minimal jerk, I found that the suggested values of spacing the spline points by 30 points was too short and sometimes it will generate too much jerk so I spaced it a little more to make the spline smoother.
By grabbing different points along the spline we can control the acceleration and max speed of the car.

To avoid collisions with other cars I first iterate over the sensor fusion results and locate the closest car ahead of us in the lane, if the car is getting close, the max speed of the vehicle is reduced to match the speed of the car ahead, If it gets even closer the speed is decreased more to keep a safe distance from it.
The logic for collision detection can be found in the file `main.cpp (lines 314-372)`
