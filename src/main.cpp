...
int pickLane(int lane, double car_s, int prev_size, const std::vector<std::vector<double>> sensor_fusion){
    ...
    // Pick best lane from possible_lanes
    for (auto i = possible_lanes.begin(); i != possible_lanes.end(); ++i){
        int current_lane = *i;
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            if(prev_size < 2){
                //use two points to make path tangent
